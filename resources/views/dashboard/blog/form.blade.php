...
@section('content')
    ...
    <div class="col-md-7 card">
        ...
        <div class="form-group">
            <label>Sort</label>
            <input type="number" class="form-control @error('sort') is-invalid @enderror" name="sort" value="@if(!empty($result->sort)){{$result->sort}}@elseif(old('sort')) {{old('sort')}} @else{{500}}@endif">
        </div>
    ...
    <div class="col-md-5 card">
        ...
        <div class="form-grop">
            <label>Video</label>
            <select name="type" id="video-type" class="form-control">
                <option value=""></option>
                @foreach ($blogtypes as $type)
                <option @if(!empty($result->type) && $type==$result->type) selected @endif value="{{$type}}">{{$type}}</option>
                @endforeach               
            </select>
        </div>

        <div class="form-group d-none" id="video-url">
